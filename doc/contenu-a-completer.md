...
| vignette            | fetch  | capture du live (microlink), sinon carte sociale GitHub (override `thumbnail`) |
...
frameworks, favicons, **vignettes**, liens, dates, badges IA, icônes Discord.
Audit des données auto effectué (liens de prod, favicons, releases, npm,
traductions, vignettes) : RAS notable. La génération des vignettes saute les
repos `ignored` et réutilise les images inchangées (clé : live + dernier push).
